...
export const executeVersionPlan = async (
  ...
  for (const action of plan.actions) {
    if (action.type === 'write-file' && action.adapter === 'package.json') await writePackageVersionFile(action.path, plan.nextVersion)
    if (action.type === 'write-file' && action.adapter === 'jsr.json') await writeJsrVersionFile(action.path, plan.nextVersion)

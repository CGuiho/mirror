...
- Added two blank lines before and two blank lines after the hello window as requested. The spacing applies only to the hello page (`mirror` with no arguments), not to any other command output.
...
- Restored the strict public previous-version command-upgrade release gate after
  3.7.2 crossed the immutable legacy-updater boundary.
- Required public 3.7.2 to upgrade itself to 3.7.3 with visible streamed
  progress, a 180-second outer deadline, canonical-version verification, and
  global-schema persistence before the release workflow can pass.
...
- Corrected the package POSIX installer schema-refresh command to use the
  canonical installed binary path outside the installer function scope.

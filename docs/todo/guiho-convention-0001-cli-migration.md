...
- Status: `todo; plan approved; execution authorized; planning-package integration pending`
...
- Authorized exact target after all implementation and merged-main gates:
  `5.0.0-alpha.0`; version commit and tag push are authorized, while GitHub
...
- Production deployment, secrets, authentication, databases, DNS, or cloud
  infrastructure.
- GitHub Release creation/publication, public lifecycle execution, or any
  production mutation. The exact `5.0.0-alpha.0` version commit and tag push
  are authorized only after the final merged-main gates.
...
- Accepted identities: CLI home `mirror`, main skill `guiho-s-mirror`, and main
  prompt `guiho-p-mirror`.
- Accepted architecture, digest-bound plan SHA-256
  `e2be85c4d361ba880022082db9d9e9054efca3176b993c3254a9ab717d137ec9`,
  and final independent plan review.
- Accepted Q-012 scoped owner/expiry precedence exception for C0001-00 through
  C0001-09; close it with the final validation evidence at expiry.
...
- C0001-00 through C0001-09 implementation, branch commits, pushes, PRs, and
  gated merges are authorized; every unit still requires its recorded clean
  base and exact-head review/validation evidence.
- Exact target `5.0.0-alpha.0`, its Mirror version commit, and tag push are
  authorized after all units and the merged-main audit. GitHub
  Release/publication remains unauthorized.

## Lifecycle State Rules

- Keep this task `todo` until the approved planning package is integrated and
  C0001-00 records its exact clean execution base.
...
- Stop before any target other than exact `5.0.0-alpha.0`, GitHub
  Release/publication, public lifecycle execution, or production mutation.
...
- A separate release TODO is created only after C0001-09 integrates and the
  refreshed merged-main audit passes. It uses the already authorized exact
  target in `mirror version plan 5.0.0-alpha.0`; GitHub Release/publication is
